...
В `React` - компонента це функція, назва якої розпочинається ВЕЛИКОЇ літери, та функція повертає `html`, який потрібно показати на екрані.
...
Як ти можеш бачити, вона виконує всі умови компоненти: це функція, назва починається з великої літери, та вона повертає HTML.
...
> Компонента це просто шаблон, який ми можемо використовувати стільки разів, скільки нам потрібно.

Для початку потрібно сказати, що будь-яка програма має один єдиний вхідний файл, з якого стартує проєкт, в випадку з `react` таким файлом є `index.js` в папці  `src`.
...
Зверни увагу на рядок `8`, тут ми використовуємо `ReactDOM`, який ми імпортували з бібліотеки `react-dom/client`, і викликаємо метод `createRoot`. Думай про це наступним чином, тут ти створюєш `React` додаток і записуєш його в константу `root`.
...
Компоненти ми використовуємо схожим чином, що й `HTML` теги. Пам'ятаєш, в `HTML` були парні теги як `div`, та непарні такі як `img`. Компоненти в `React` використовуються як не парні теги:
...
Перше, що я хочу обговорити, я не раз говорив, про те, що `import` є тільки в `Node.js`, і браузер не вміє то читати, тому ми збираємо весь javascript до купи та кладемо в один єдиний файл, щоб браузер міг то прочитати, тоді питання як це все зібрати до купи?
