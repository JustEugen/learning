...
Як ти можеш бачити, в нашому білді в нас згенерувався `css` файл, який автоматично підключився до `index.html` і він містить саме той `css` який ми підключали в середині нашої `App` компоненти на першому рядку за допомогою `import './mystyles.css';`

Штука яка білдає проект, називається `webpack` - він крутий тим, що його можна розширяти, додавши туди якийсь плагін, по суті якийсь кусок коду який щось робить. Ну і розробники реакту такі подумали, і бляха, нам треба зробити роботу з `CSS` простішою, давайте щось думати.
...
Потрібно дістати весь той `CSS`, відложити його, ось цю строку видалити, а видалити потрібно томущо ти не можеш писати css в javascript файлі, бо це як мінімум різні мови, з абсолютно різним синтаксисом. Вот він взяв, цей `CSS` відложив його, і коли білд `javascript'а` закінчиться, створити `CSS` файл, запихнути туди всі ті стилі які він відложив, і підключити його в `index.html`.
...
Суть CSS Modules в тому, щоб для кожної компоненти, генерувалася унікальна назва для CSS класу, тобто в тебе в проекті може бути хоч мілліон компонент, які в своїх CSS файлах будуть мати клас `.hello`, але в кожної компоненти, він буде називатися по іншому.

Тепер, якщо повернутися до проблематики. Якщо ти наприклад, працюєш на величезному проекті, і в середині CSS Module (наприклад A.module.css) змінив, або додав якийсь css клас, ця зміна відобразиться тільки на цій конкретній компоненті
...
Також навіть якщо щось ти не розумієш зараз, коли ти сам будеш пробувати з цим працювати, 
