...
Якщо під час виконання коду, Javascript рушій натикається на створення змінної, він запише данні про неї в поточний scope. Тоді, коли він побачить, що десь використовується певна змінна, він почне шукати її лише в середині одного єдиного місця, а саме в середині scope.
...
3. В `console.log` ми намагаємося вивести змінну `test`, тому javascript рушій почне шукати, чи є така змінна в поточному скоупі, а поточний скоуп, в даному випадку, це скоупу функції `sayHello`. В нашому випадку така змінна є, і в консоль нам виведеться: 'I am inside function'

### Блочний scope / block scope
Block scope існує в середині тіла if else, while, do while, for, try catch і працює він тільки для `let` та `const`, `var` ігнорує блочинй скоуп, і створюється або в глобальному, або в локальному. 

> Про var буде детальніше в прикладах внизу, але мушу сказати, що зараз ми не використовуємо `var` багатьох причинах, одна з них те, що вона не бачить block scope. Також скажу, що не бачить вона block scope, так як взагалі поняття block scope в javascript прийшло в 2015 році, а var існували з моменту створення javascript.
