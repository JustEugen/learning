...
Одразу скажу, що писати код можна і без функцій, але в такому випадку, ти отримаєш дуже багато дублювання. Основне завдання функцій - це зберігання коду в певній структурі, яку ти зможеш вснути туди куди тобі потрібно, і код всередині неї виконається.
...
3. Все завжди змінюється, особливо в IT сфері, тому є ризики, що цю формулу потрібно буде також змінювати, в даному варіанті, зробити це буде дуже важко, томущо ти не зможеш виконати якийсь глобальний пошук, по причині, що в тебе не має ніяких ключових слів, за які ти міг би вхопитися.

### Рішення проблеми організації і зміни формули

Давай створимо функцію, і запихнемо цю калькуляцію в неї, і будемо просто перевикористовувати функцію. Для того щоб все виглядало більш реалістично, я придумаю рандомні назви для всих цих значень

Для таких речей, і стоврені функції, зменшити дублювання коду організувавши його в певну структуру.
```js
function calculateOffset() {
  (10 + 30) / 60 * 89
}
```
Ось ми створили нашу функцію, `calculateOffset` - яка буде займатися обрахунками певного офсету.

Для того щоб код в середині функції виконався, нам потрібно викликати нашу функцію.
```js
calculateOffset();
```
...
calculateOffset();
```
Отже, на даному етапі ми вирішили пробелему організації нашого коду, тепер, якщо нам щось потрібно буде змінити в нашій формулі, ми просто змінимо її в середині функції. Також частково ми вирішили пробелму дублювання коду.
...
**Аргументи функції** - це те що нам потріно, за допомогою них, ми можемо для кожного виклику нашої функції, передавати різні значення.


Аргументи функції передаються під час її виклику, через кому, в якості аргумента може бути будь що, number, string, object і навіть інша функція. Я наведу детальний приклад:
```js
// Як було до функцій, хочу щоб ти бачив різницю
const drillingOffset = (10 + 30) / 60 * 89; 
const checkOffset = (70 + 12) / 77 * 12;
...
Як можмео бачити на прикладі, ми передаємо в функцію аргументи, через кому, які необхідні для обрахунків, але в мене є 2 питання:
